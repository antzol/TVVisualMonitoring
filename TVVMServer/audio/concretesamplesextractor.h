...
{
public:
    std::shared_ptr<std::vector<int16_t>> getNextSamplesForAllChannels() override;
...
//---------------------------------------------------------------------------------------
template<typename T>
std::shared_ptr<std::vector<int16_t>> ConcreteSamplesExtractor<T>::getNextSamplesForAllChannels()
{
    ...
    else
    {
        for (int i = 0; i < channelsCount && i < samples->size(); ++i)
        {
            ...
            {   // for float
                convertedValue = std::clamp(static_cast<float>(sampleValue), -1.0f, 1.0f) * 0x7FFF;
            }
